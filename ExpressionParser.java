...
public class ExpressionParser {
    ...
    private double calculate(String equation) {
        ...
        // we have to finish the leftover calls the multiplication one so we do left times right
        ...
        // sin(0.5+0.5) needs to be recognised as a function call, not scanned for + operators
        // the pattern is: does the string start with "functionname(" and end with ")"?
        // if yes, and the closing ) belongs to this function (not something nested inside),
        // then we strip the function name, evaluate the inside, and apply the math function
        ...
        for (String fn : funcs) {
            if (equation.startsWith(fn + "(") && equation.endsWith(")")) {
                ...
                // we do this by finding the matching ) for the opening ( right after the fn name
        ...
        // this comes after operator scanning so we dont accidentally treat a binary minus as unary
    ...
    // given the index of an opening '(' in the string, finds the index of its matching ')'
    // we need this to check whether the closing ) of something like sin(...) actually
    // belongs to that function call or is just coincidentally at the end of the string
