...
public class CourseConfigurationProfile implements CourseProfile {
    ...
    public double getHeight(double x, double y) {
        return config.heightFunction.evaluate(x, y);
    }

    @Override
    public double getSlopeX(double x, double y) {
        return config.heightFunction.dhdx(x, y);
    }

    @Override
    public double getSlopeY(double x, double y) {
        return config.heightFunction.dhdy(x, y);
    }
    ...
    public double[] getStartPosition() {
        return new double[] { config.startX, config.startY };
    }

    @Override
    public double[] getTargetPosition() {
        return new double[] { config.targetX, config.targetY };
    }

    @Override
    public double getTargetRadius() {
        return config.targetRadius;
    }
